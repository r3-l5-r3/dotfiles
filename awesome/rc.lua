...
-- hack to skip loading naughty (prevent conflict w/ dunst)
...
local cmpt = "compton -CGf -D3 -O0.05"
...
awful.spawn.with_shell(cmpt)

-- {{{ Variable definitions
-- Themes define colours, icons, font and wallpapers.
beautiful.init(os.getenv("HOME") .. "/.config/awesome/themes/base16-atelier-cave/theme.lua")
...
local lockscreen = "csxlock -f -*-gohufont-*-*-*-*-17-*-*-*-*-*-*-* -b '#19171c' -o '#576ddb' -w '#be4678'"
--local quit = "pkill -15 -t tty'$XDG_VTNR' Xorg"
...
myawesomemenu = {
	...
	{ "quit to tty", function() awesome.quit() end}
}
...
-- {{{ Wibar
-- No-bar Spacer textbox widget
nobarspacer = wibox.widget.textbox()
nobarspacer.text = "  "
...
-- Network usage additional info tooltip (WIP)
--netwidget_t = awful.tooltip({
--	mode = "outside",
--	tooltip_align = "top_left",
--	objects = { netwidget },
--	timer_function = function()
--	text = vicious.call(vicious.widgets.wifi("wlp2s0"), function(widget, args)
--		return string.format("%.8s - %sdB", args["{ssid}"], args["{sign}"])
--		end, 19, "wlp2s0")
--	end,
--})

-- Create a textclock widget + calendar popup widget
mytextclock = wibox.widget.textclock()
month_calendar = awful.widget.calendar_popup.month{ bg="#00000000", font=beautiful.font, margin=3, start_sunday=true, long_weekdays=true }
...
-- Register widget
vicious.register(memwidget, vicious.widgets.mem, '<span color="#955ae7">MEM: $1%</span>', 13)
...
-- Register widget
vicious.register(cpuwidget, vicious.widgets.cpu, '<span color="#398bc6">CPU: $1%</span>')
...
--        {description = "go back", group = "client"}),
    ...
    awful.key({                   }, "Print", function () awful.util.spawn("sh .maim-fullshot") end,
	      {description = "Take screenshot (fullscreen)", group = "utilities"}),
    awful.key({ modkey,           }, "Print", function () awful.util.spawn("sh .maim-windowshot") end,
	      {description = "Take screenshot (window)", group = "utilities"}),
    awful.key({ modkey, "Shift"   }, "Print", function () awful.util.spawn("sh .maim-monitorshot") end,
	      {description = "Take screenshot (current monitor)", group = "utilities"}),

    awful.key({ modkey,           }, "XF86MonBrightnessDown", function () awful.spawn(lockscreen) end,
    ...
    awful.key({ modkey, "Mod1"    }, "XF86MonBrightnessDown", function () awful.spawn("pkill compton") end,
	      {description = "Kill compton", group = "utilities"}),
    awful.key({ modkey, "Mod1"    }, "XF86MonBrightnessUp", function () awful.spawn(cmpt) end,
	      {description = "Start compton", group = "utilities"}),
...
for i = 1, 9 do
    if i > 4 then -- Modify code to stop at 4, since there are only 4 tags in this config!
        break
    end
...
awful.rules.rules = {
    ...
    { rule = { },
     ...
     callback = awful.client.setslave -- start clients as slave windows
    },
    ...
    { rule_any = {
	  ...
	  "Gnome-mines", -- Mines
        ...
        name = {
          "Event Tester",  -- xev (keycode tester)
        },
        role = {
    ...
    -- No titlebars on normal clients and dialogs
    { rule_any = { type = { "normal", "dialog" }
      }, properties = { titlebars_enabled = false }
    },

    -- Set Firefox to always open as a master window
    { rule = { class = "Firefox", role = "browser" },
      callback = awful.client.setmaster
      },

    -- Set Discord, Pidgin and Steam Friends List to always map on the tag named " :"
    ...
    -- Set Thunderbird to always map on the tag named ".:"
    { rule = { class = "Thunderbird", role = "3pane" },
      properties = { tag = ".:", switchtotag = true }
      },
    	-- Set password prompts to appear on current tag
--	{ rule { class = "Thunderbird", instance = "Dialog" },
--	  properties = { placement = "centered, tag = function() return awful.screen.focused().selected_tag end }
--	},

    -- Set Streamlink Twitch GUI, Steam, Lutris to always map on the tag named "::"
    { rule_any = { class = { "streamlink-twitch-gui", "Lutris" },
    		   name = { "Steam" } },
      except = { name = "Steam Login" },
      properties = { tag = "::" }
      }
